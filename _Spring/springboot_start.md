...
tomcatEmbeddedServletContainerFactory（实现了 ErrorPageRegistry 接口）初始化时

ErrorPageRegistrarBeanPostProcessor会从beanFactory中使用类型获取到所有的 ErrorPageRegistrar.class，以 ErrorPage 对象的形式全部注册到tomcatEmbeddedServletContainerFactory中

TomcatEmbeddedServletContainerFactory.getEmbeddedServletContainer时将定义的 ErrorPage，传递给StandardContext

请求异常后，
StandardHostValve 设置 request中 org.apache.catalina.core.DISPATCHER_REQUEST_PATH -> /error 
```
